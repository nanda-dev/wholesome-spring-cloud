...
public class AccountsServiceImpl implements AccountsService {
	...
	public AccountsServiceImpl(ModelMapper mapper, AccountsRepository accountsRepo, 
	...
	public AccountModel createAccount(AccountModel accountModel) {
		...
		log.info("Checking whether Age [{}] is less than minimum age accepted by the system: [{}].", age, createAccountValidationProperties.getMinAge());
		
		if(age < createAccountValidationProperties.getMinAge()) {
			throw new RuntimeException("Age cannot be less than " + createAccountValidationProperties.getMinAge());
		}
	...
	public List<AccountModel> getAllAccounts() {
		...
		return accounts.stream().map(account -> mapper.map(account, AccountModel.class)).collect(Collectors.toList());
	}
